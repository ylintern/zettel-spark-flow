...
User creates note in ViBo → writes to disk + inserts into SQLite
User opens same note in Obsidian on another device → modifies .md file directly
User returns to ViBo → which version is authoritative?
...
Repo path/structure (or zip of src/commands and src/face)
Answers to Section B checklist (which commands exist, which don't)
Current Cargo.toml (to see which crates are actually used)
A screenshot of the current Phase 0 UI (to see what's already built)
